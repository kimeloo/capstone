...
# 활용할 모델 선택
model = LogisticRegression(max_iter=2000, random_state=2024)

# 데이터 불러오기
pwd = '~/documents/coding/capstone/data/240201/'
data = pd.read_csv(pwd + '03_htnderv_s1_train.csv')

# 데이터 확인
print(data['htnderv_s1'].value_counts(), end='\n\n')
...
data['PropensityScore'] = model.predict(X)      # lightgbm의 경우 predict_proba가 없음
...
# 중복 개수 확인
print(f'중복 개수: {matched_data.duplicated().sum().sum()}')

# 결과 확인
# print(matched_data.head())

# htnderv_s1가 0인 데이터와 1인 데이터의 개수 확인
print(matched_data['htnderv_s1'].value_counts(), end='\n\n')
