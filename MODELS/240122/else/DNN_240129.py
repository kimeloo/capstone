...
X = X[['poremlat', 'cvcer', 'poremli', 'popcsa95', 'm1adepr', 'cvtia', 'pqpenth', 'fosogact', 'podsrem4', 'fosofam1']].copy()
...
model = Sequential([
    ...
    # Conv1D(256, kernel_size=3, strides=2, activation='relu'),
    # Flatten(),
...
for i in range(2):
    for j in range(2):
        plt.text(j, i, str(conf_mat[i, j]), ha='center', va='center', color='red', fontsize=20)
plt.show()